...
express::Router createApiRouter(database::mariadb::MariaDBClient &db) {
  ...
  apiRouter.get("/users", [&db] APPLICATION(req, res) {
    ...
    db.query(
        ...
        [&res](const std::string &errorString, unsigned int errorNumber)
            -> void { handleDbError(res, errorString, errorNumber); });
  });
        ...
        [&res, &db](nlohmann::json &body) -> void {
          if (body["title"] != nullptr) {
            db.query(
                ...
                "VALUES( '" +
                    string{body["title"]} + "'" + string{body["topicUuid"]} +
                    "'" + string{body["user_account_id"]} + "' )",
                [&res, &db, &body](const MYSQL_ROW row) -> void {
                  // DO SOMETHING
                  json *threadJson = new json; // needs to be within
                  db.query(
                      "select * from view_threads where thread_title = '" +
                          string{body["title"]} +
                          "'"
                          "AND user_acount_id= '" +
                          string{body["user_account_id"]} + "'",
                      [&res, &db, threadJson](const MYSQL_ROW row) -> void {
                        if (row != nullptr) {
                          threadJson->push_back(
                              {{"uuid", row[1]},
                               {"title", row[2]},
                               {"created_at", row[3]},
                               {"user",
                                {{"uuid", row[6]}, {"username", row[7]}}}});
                        } else {
        ...
        [&res, &db](nlohmann::json &body) -> void {
          if (body["content"] || body["threadUuid"] != nullptr) {
            db.query(
                ...
                "VALUES( '" +
                    string{body["content"]} + "'" + string{body["threadUuid"]} +
                    "'" + string{body["user_account_id"]} + "' )",
                [&res, &db, &body](const MYSQL_ROW row) -> void {
                  // DO SOMETHING
                  ...
                  db.query(
                      "select * from view_comments where comment_content = '" +
                          string{body["content"]} +
                          "'"
                          "AND user_acount_id= '" +
                          string{body["user_account_id"]} + "'",
              ...
              [&res, &db, &body](const MYSQL_ROW row) -> void {
                if (row != nullptr) {
                  ...
                  if (count == 1) {
                    db.query(
                        ...
                        [&res, &body](const MYSQL_ROW row) -> void {
                          if (row != nullptr) {
                            ...
                            // Check password
                            if (dbPasswordHash ==
                                hashSha1(dbPasswordSalt + bodyPassword)) {
                              ...
                              res->status(401).send(
                                  json{{"error", "Invalid password"}}.dump(4));
                            }
                          }
                        },
                        [&res](const std::string &errorString,
                    ...
                    VLOG(0) << "No user exists with the email '"
                            << body["email"] << "'";
                    res->status(404).send(
                        json{{"error", "Email does not exist"}}.dump(4));
                  }
                }
              },
              [&res](const std::string &errorString, unsigned int errorNumber)
  ...
  apiRouter.get("/topic/:uuid", [&db] APPLICATION(req, res) {
    db.query(
        ...
        [&res, &db](const MYSQL_ROW row) -> void {
          ...
          db.query(
              ...
              "where thread_topic_id = '" +
                  string{char(topicId)} + "'",
              [&res, threadFound, topicFound,
               topicThreads](const MYSQL_ROW row) -> void {
                if (row != nullptr) {
                  int topicId{stoi(row[0])}; // topic Id
                  threadFound->push_back(
                      {"threads",
                       {{"uuid", row[1]},
                        {"title", row[2]},
                        {"created_at", row[3]},
        ...
        [&res](const std::string &errorString, unsigned int errorNumber)
            -> void { handleDbError(res, errorString, errorNumber); }); // query
  }); // get request
    ...
    db.query(
        ...
        [&res, &db](const MYSQL_ROW row) -> void {
          ...
          db.query(
              ...
              "where thread_topic_id = '" +
                  string{char(topicId)} + "'",
              [&res, threadJson, commentFound,
               threadComments](const MYSQL_ROW row) -> void {
                if (row != nullptr) {
                  int topicId{stoi(row[0])}; // topic Id
                  threadJson->push_back(
                      {"threads",
...
void handleDbError(const std::shared_ptr<express::Response> &res,
